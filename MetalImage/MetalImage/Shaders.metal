...
struct AdjustStaturationUniforms
{
...
kernel void adjust_saturation(texture2d<float, access::read> inTexture[[texture(0)]],
                              ...
                              constant AdjustStaturationUniforms & uniforms[[buffer(0)]],
                              uint2 gid[[thread_position_in_grid]])
{
    float4 inColor = inTexture.read(gid);
    float value = dot(inColor.rgb, float3(0.299, 0.587, 0.014));
                             ...
                             uint2 gid[[thread_position_in_grid]])
{
    ...
    outTexture.write(float4(accumColor.rgb, 1.0f), gid);
}